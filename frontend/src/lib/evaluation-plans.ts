...
function twoStepRules(variant: EvaluationVariantId): EvaluationPlanRules {
  const maxLoss = variant === "STANDARD" ? 10 : variant === "PRO" ? 8 : 12;
  return {
    profitTargetPhase1: 10,
...
export function formatEvaluationSize(size: number): string {
  if (size >= 1000) {
    const k = size / 1000;
...
export function getPlansForSelection(
  ...
  if (type === "TWO_STEP") {
    return EVALUATION_PLANS.find((p) => p.type === type && p.variant === variant);
  }
  return EVALUATION_PLANS.find((p) => p.type === type);
}
