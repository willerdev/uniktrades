...
export function ProfitSharePaymentPanel({
  feeUsdt,
  onComplete,
}: {
  feeUsdt: number;
  ...
  async function startPayment() {
    ...
    setError("");
    try {
      const result = await api.payments.createProfitShare(network);
      if (result.active) {
        onComplete?.();
        return;
      }
    ...
    return (
      <div className="space-y-4 rounded-lg border border-[var(--color-border)] bg-foreground/[0.02] p-4">
        <p className="text-sm text-muted">
          ...
          to activate profit share — 50% of setup wins and copy-trade commissions.
        </p>
    ...
    <div className="space-y-4 rounded-lg border border-primary/30 bg-primary/5 p-4">
      ...
      {progress === "complete" ? (
        <div className="flex items-center gap-2 text-success">
          ...
          <span className="text-sm font-medium">Profit share activated.</span>
        </div>
      ) : (
        <>
          ...
          <p className="flex items-start gap-2 text-xs text-muted">
            ...
            We check every 8 seconds and activate profit share when payment confirms.
          </p>
        </>
      )}
    </div>
